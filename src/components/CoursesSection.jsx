        ...
        {loading ? (
          ...
          <div className="flex flex-col space-y-32 md:space-y-48">
            {activeCourses.map((course, idx) => {
              ...
              const bgImage = course.image_url || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1200&auto=format&fit=crop';
              
              return (
                <div key={course.id} className={`relative flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} gap-12 lg:gap-24 items-center`}>
                  
                  {/* Edit Button overlay */}
                  <EditButton 
                    onClick={() => navigate('/gestore-forme-2026')} 
                    className={`top-0 ${isEven ? 'left-0' : 'right-0'}`} 
                  />
